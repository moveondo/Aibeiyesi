...
class Load2DB():
    ...
    def deleteData(self,dbName,corp_analyze_id,model_name):
        ...
        sql = """delete from %s where corp_analyze_id=%s and TYPE='ai_annotation' and model_type='%s'""" %(dbName,corp_analyze_id,model_name)
                ...
                if abs(contents_dict['start_offset']-end_offset_tmp)<5:
                    ...
                    if over:
                        ...
                        if end_offset_tmp-start_offset_tmp>50:#大于50的字段存入
                            ...
                            source_str = contents_dict['text']
                            if contents_dict['tag_class_id'] not in aspectId_dict.keys():
                                aspectId = self.getAspectId(bm_id,contents_dict['tag_class_id'])
                                aspectId_dict[contents_dict['tag_class_id']] = aspectId
                            else:
                                aspectId = aspectId_dict[contents_dict['tag_class_id']]
